...
test("bonjour.find", function(bonjour, t) {
  ...
  bonjour
    .publish({ name: "Foo Bar", type: "test", port: 3000 })
    .on("up", next());
  bonjour
    .publish({ name: "Invalid", type: "test2", port: 3000 })
    .on("up", next());
  bonjour
    .publish({ name: "Baz", type: "test", port: 3000, txt: { foo: "bar" } })
    .on("up", next());
});
...
test("bonjour.findOne - callback", function(bonjour, t) {
  ...
  bonjour
    ...
    .on("up", next());
  bonjour
    .publish({ name: "Callback", type: "test", port: 3000 })
    .on("up", next());
});
...
test("bonjour.findOne - emitter", function(bonjour, t) {
  ...
  bonjour
    .publish({ name: "Emitter", type: "test", port: 3000 })
    .on("up", next());
  bonjour
    .publish({ name: "Invalid", type: "test2", port: 3000 })
    .on("up", next());
